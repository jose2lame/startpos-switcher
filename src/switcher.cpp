...
std::vector<std::pair<StartPosObject*, CCPoint> >
    switcher::startPoses = {};
std::array<enumKeyCodes, 2>
    switcher::keyBinds = {KEY_Left, KEY_Right};
...
CCMenuItemSpriteExtra* 
    switcher::leftArrowButton = nullptr;
CCMenuItemSpriteExtra* 
    switcher::rightArrowButton = nullptr;
bool switcher::isInMenu = false;
bool switcher::isCreated = false;

namespace {
    void pulseObjects(void) {
        switcher::leftArrow->runAction(
            CCSequence::create(CCFadeTo::create(0.0f, 255), 
                               CCFadeTo::create(1.0f, 255), 
                               CCFadeTo::create(0.5f, 70),
            nullptr));
        switcher::rightArrow->runAction(
            CCSequence::create(CCFadeTo::create(0.0f, 255), 
                               CCFadeTo::create(1.0f, 255), 
                               CCFadeTo::create(0.5f, 70), 
            nullptr));
...
void switcher::switchStartPos(PlayLayer* game) {
    ...
    if (switcher::startPoses.empty() || 
        game->m_isPracticeMode) 
        return;
    ...
    if (switcher::index == -1) {
        ...
        game->m_startPos = 
            switcher::startPoses[switcher::index].first;
        game->m_playerStartPosition = 
            switcher::startPoses[switcher::index].second;
    }
...
void switcher::Callbacks::leftArrowButtonCallback(
    CCObject* obj
) {
...
void switcher::Callbacks::rightArrowButtonCallback(
    CCObject* obj
) {
    auto pPlayLayer = PlayLayer::get();
